...
function Shipper() {
    ...
    const [shipper, setShipper] = state.userAPI.allShipper
    const [shipperCopy] = state.userAPI.allShipperCopy
    ...
    const search = (e) => {
        const matchedUser = shipperCopy.filter((user) => {
    ...
    useEffect(() => {
        ...
        setCurrentItems(shipper.slice(itemOffset, endOffset));
        setPageCount(Math.ceil(shipper.length / itemsPerPage));
    }, [itemOffset, itemsPerPage, shipper])

    const handlePageClick = (event) => {
        const newOffset = (event.selected * itemsPerPage) % shipper.length;
    ...
    return (
        <div>
            ...
            <div className="content-wrapper">
                ...
                <div className="search-user">
                    <Link className="add-staff-link" to='/shipper/addshipper'>+ Add Shipper</Link>
                </div>
                        ...
                        <tbody className="table-body">
                            {
                                currentItems.map(user => (
                                    <tr key={user._id}>
                                        ...
                                        <td>{user.role === 2 ? 'Shipper' : 'Customer'}</td>
                                        <td>
